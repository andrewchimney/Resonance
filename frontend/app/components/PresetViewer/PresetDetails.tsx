...
/**
 * PresetDetails - Visual display of all preset parameters
 *
 * Renders a read-only (for now) breakdown of a Vital preset including
 * oscillators, envelopes, filters, LFOs, effects, macros, and modulation.
 */
...
export function StatusIndicator({
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className={`w-2.5 h-2.5 rounded-full transition-colors ${active ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
      <span className={`text-[10px] transition-colors ${active ? 'text-zinc-300' : 'text-zinc-600'}`}>
        {label}
      </span>
    </div>
  );
...
// ============================================
// WAVEFORM HELPERS  (mirrors PresetViewer)
// ============================================
...
function WaveformMini({ waveData, enabled }: { waveData?: string; enabled: boolean }) {
  ...
  const pathData = useMemo(() => {
    ...
    const points: number[] = [];
    for (let i = 0; i < samples.length; i += step) points.push(samples[i]);
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      const y = padding + ((1 - points[i]) / 2) * usableHeight;
  ...
  return (
    <div className={`p-3 rounded-lg border ${
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className={`w-2.5 h-2.5 rounded-full ${oscillator.enabled ? 'bg-cyan-400' : 'bg-zinc-700'}`} />
      </div>

      {/* Waveform */}
      <div className="w-full mb-2">
        <WaveformMini waveData={oscillator.wavetable?.waveData} enabled={oscillator.enabled} />
      </div>

      {/* Wavetable name */}
      <span className={`text-xs truncate block mb-1 ${oscillator.enabled ? 'text-zinc-200' : 'text-zinc-500'}`}
            title={oscillator.wavetableName}>
        {oscillator.wavetableName}
      </span>

      {/* Component type sub-label */}
      {oscillator.wavetable?.componentType && oscillator.wavetable.componentType !== 'Wave Source' && (
        <span className="text-[10px] text-zinc-500 block truncate mb-2">
          {oscillator.wavetable.componentType
            .replace('Audio File Source', 'Sample')
            .replace('Wave Warp', '+Warp')}
        </span>
      )}
  ...
  return (
    <div className={`flex-1 ${!filter.enabled && 'opacity-40'}`}>
      <div className="flex items-center gap-1.5 mb-2">
        <div className={`w-2 h-2 rounded-full cursor-pointer ${filter.enabled ? 'bg-blue-400' : 'bg-zinc-600'}`} />
  ...
  return (
    <div className="w-full bg-zinc-900 rounded-xl border border-zinc-700 overflow-hidden font-sans">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex gap-3 flex-wrap">
          {/* Filters */}
          <div className="flex-1 min-w-[160px] p-3 bg-zinc-800/50 rounded-lg border border-zinc-700">
